...
def generate_coach_advice(client, candidate_models, report_text):
    ...
    if is_weekend:
       ...
       - 菜單區塊標題請明確使用：🎯 【今日具體訓練菜單 (假日彈性時段指引)】"""
        ...
        menu_instruction = f"""6. 【目標賽事備賽週期與今日訓練菜單 (今日為{weekday_str}平日，晚間執行)】：
       - 跑者作息說明：跑者平日固定於「晚間/夜間」跑步訓練。本報表於清晨送達，供跑者在展開新的一天時提前掌握今晚目標與日間準備。
       - 請檢視「未來目標賽事」及其倒數天數/週數（例如：倒數 8 週代表處於基礎耐力期、倒數 4 週進入專項強度期、倒數 2 週進入賽前減量 Taper 期）。
       - 綜合當前賽事備賽週期、ACWR 負荷比與最新生理恢復狀況，為「今晚」開立最合適的具體課表。
       - 請務必指定精確的訓練目的（E 輕鬆跑 / M 馬拉松配速 / T 節奏跑 / I 間歇 / 完全休息）、預計公里數、丹尼爾靶心配速範圍，並附上暖身與收操提醒。
       - 晚跑專屬貼心指引：
         * 【日間儲備】：提醒下午水分補充、午餐優質碳水儲備，以及跑前 1.5~2 小時輕食補給。
         * 【疲勞調控與夜跑收操】：提醒下班後以心率/自覺量表 (RPE) 抗衡整日工作疲勞（切勿盲目追求死板配速）；若今晚為強度課表，提醒跑後徹底冷卻收操與肌肉放鬆，避免影響夜間睡眠品質與自律神經修復。
       - 菜單區塊標題請明確使用：🎯 【今日具體訓練菜單 (今晚執行)】"""
...
def generate_rest_day_advice(client, candidate_models, report_text):
    ...
    if is_weekend:
       ...
       - 跑者作息說明：跑者平日固定於「晚間」訓練。今日排定為休整日，請以跑者「今日日間/晚間休整安排」與「明晚重啟課表」為視角。
       - 檢視是否有「未來目標賽事」及其倒數天數/週數，判斷當前備賽週期階段。結合跑者的體能恢復節奏與「丹尼爾 VDOT 靶心配速指針」，開立具體明確的明晚重啟訓練菜單（指定 E/M/T/I/R 訓練目的、精確公里數與靶心配速，例如安排輕量 E 配速跑 5~6km 喚醒神經肌肉，或具備重啟 Tempo / 間歇質量課表的條件）。標題請使用：🎯 【今日休整指引與重啟課表預告】"""
