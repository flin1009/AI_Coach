...
def generate_coach_advice(client, candidate_models, report_text):
    ...
    數據包含兩部分：
    A. 【近期訓練歷程 (近 7 筆摘要)】：提供近一週的累積跑量、交叉訓練（肌力/單車等）頻率與急劇疲勞脈絡。
    B. 【最新活動深度數據】：包含最新一筆活動的分圈、心率區間 (Z1~Z5)、跑步動態、功率與環境氣象。
    ...
    6. 【綜合 7 日週期課表建議】：縱觀「近期訓練歷程 (近 7 筆摘要)」中的累積跑量、運動頻率與急劇疲勞，具體為今天/明天給出最合適的訓練或休整建議（例如：安排主動恢復跑、完全休息或安排質量課表）。
