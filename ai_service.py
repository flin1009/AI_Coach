...
def generate_coach_advice(client, candidate_models, report_text):
    ...
    D. 【近期訓練歷程 (近 7 筆摘要)】：提供近一週的累積跑量、交叉訓練頻率與急劇疲勞脈絡。
    E. 【最新活動深度數據】：包含最新一筆活動的分圈、心率區間 (Z1~Z5)、前後半程有氧解耦率 (Decoupling %)、跑步動態、功率與環境氣象。
    ...
    6. 【綜合 ACWR 負荷與明日具體課表處方】：結合 ACWR 負荷指數與今日生理恢復狀況，具體為明天開立最合適的訓練處方。請務必指定精確的公里數與丹尼爾靶心配速範圍（例如：「建議進行 6~8km E 配速跑 (5:45~6:15/km)，心率維持在 Zone 2」，或安排完全休息）。
...
def generate_rest_day_advice(client, candidate_models, report_text):
    ...
    4. 【下次重啟訓練課表預告】：依據跑者的體能恢復節奏與「丹尼爾 VDOT 靶心配速指針」，具體預告明天或下次重啟訓練時，建議執行何種課表，並指定精確的公里數與丹尼爾靶心配速（例如：安排輕量 E 配速跑 5~6km 喚醒神經肌肉，或具備重啟 Tempo / 間歇質量課表的條件）。
