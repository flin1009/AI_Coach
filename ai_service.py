...
def generate_coach_advice(client, candidate_models, report_text):
    ...
    6. 【目標賽事備賽週期與今日訓練菜單 (晚間執行)】：
       - 跑者作息說明：跑者固定於「晚間/夜間」跑步訓練。本報表於清晨送達，供跑者在展開新的一天時提前掌握今晚目標與日間準備。
       ...
       - 晚跑專屬貼心指引：
         ...
         * 【夜跑收操】：若今晚為強度課表，提醒跑後徹底冷卻收操與肌肉放鬆，避免影響夜間睡眠品質與自律神經修復。
...
def generate_rest_day_advice(client, candidate_models, report_text):
    ...
    4. 【目標賽事備賽週期與今日休整/下次重啟訓練菜單】：
       - 跑者作息說明：跑者固定於「晚間」訓練。今日排定為休整日，請以跑者「今日日間/晚間休整安排」與「下次/明晚重啟課表」為視角。
       - 檢視是否有「未來目標賽事」及其倒數天數/週數，判斷當前處於備賽哪一週期階段。結合跑者的體能恢復節奏與「丹尼爾 VDOT 靶心配速指針」，開立具體明確的下次/明晚重啟訓練菜單（指定 E/M/T/I/R 訓練目的、精確公里數與靶心配速，例如安排輕量 E 配速跑 5~6km 喚醒神經肌肉，或具備重啟 Tempo / 間歇質量課表的條件）。標題請使用：🎯 【今日休整指引與重啟課表預告】
