...
def generate_rest_day_advice(client, candidate_models, report_text):
    ...
    4. 【目標賽事備賽週期與今日休整/下次重啟訓練菜單】：特別注意此報表在清晨發送，請以跑者「今日休整安排」與「下次重啟課表」為視角。檢視是否有「未來目標賽事」及其倒數天數/週數，判斷當前處於備賽哪一週期階段。結合跑者的體能恢復節奏與「丹尼爾 VDOT 靶心配速指針」，開立具體明確的下次/明日重啟訓練菜單（指定 E/M/T/I/R 訓練目的、精確公里數與靶心配速，例如安排輕量 E 配速跑 5~6km 喚醒神經肌肉，或具備重啟 Tempo / 間歇質量課表的條件）。標題請使用：🎯 【今日休整指引與重啟課表預告】
