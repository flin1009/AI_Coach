...
def generate_coach_advice(client, candidate_models, report_text):
    ...
    6. 【目標賽事備賽週期與明日具體訓練菜單】：
       - 請特別檢視「未來目標賽事」及其倒數天數/週數（例如：倒數 8 週代表處於基礎耐力期、倒數 4 週進入專項強度期、倒數 2 週進入賽前減量 Taper 期）。
       - 綜合當前賽事備賽週期、ACWR 負荷比與今日生理恢復狀況，為明天開立最合適的「具體訓練菜單」。
...
def generate_rest_day_advice(client, candidate_models, report_text):
    ...
    4. 【目標賽事備賽週期與明日/重啟訓練菜單】：檢視是否有「未來目標賽事」及其倒數天數/週數，判斷當前處於備賽哪一週期階段。結合跑者的體能恢復節奏與「丹尼爾 VDOT 靶心配速指針」，開立具體明確的下次訓練菜單（指定 E/M/T/I/R 訓練目的、精確公里數與靶心配速，例如安排輕量 E 配速跑 5~6km 喚醒神經肌肉，或具備重啟 Tempo / 間歇質量課表的條件）。
